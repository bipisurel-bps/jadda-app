...
interface SafarChapter { id: number; title: string; icon: string; sections: SafarSection[]; }
interface SafarData { title: string; description: string; source: string; note?: string; chapters: SafarChapter[]; }
...
export default function FiqhSafarClient() {
  ...
  useEffect(() => {
    fetch('/data/fiqh-safar.json').then(r => r.json()).then(d => { setData(d); setLoading(false); }).catch(() => setLoading(false));
  }, []);
    ...
    <div className="min-h-screen bg-[#050a14]">
      ...
      <div className="max-w-2xl mx-auto px-4 pb-12 space-y-5">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          className="rounded-2xl bg-white/[0.03] border border-cyan-400/10 p-4">
          <div className="flex items-start gap-3">
            <BookOpen size={16} className="text-cyan-400 mt-0.5 shrink-0" />
            <div>
              <p className="text-xs font-extrabold text-cyan-400 mb-1 uppercase tracking-tight">Sumber Rujukan</p>
              ...
              <div key={chapter.id} className="rounded-2xl bg-white/[0.03] border border-white/[0.06] overflow-hidden">
                <button onClick={() => setExpandedChapters(prev => { const n = new Set(prev); n.has(chapter.id) ? n.delete(chapter.id) : n.add(chapter.id); return n; })}
                  ...
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${isOpen ? 'bg-cyan-500/10 border border-cyan-400/20' : 'bg-white/[0.03]'}`}>
                      <BookOpen size={15} className={isOpen ? 'text-cyan-400' : 'text-white/35'} /></div>
                    <div className="text-left">
                      <h3 className={`font-extrabold text-sm ${isOpen ? 'text-cyan-400' : 'text-white/85'}`}>Bab {chapter.id}: {chapter.title}</h3>
                ...
                <AnimatePresence>
                  {isOpen && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.2 }}>
                      <div className="p-4 space-y-5"><SafarSectionRenderer sections={chapter.sections} /></div>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
...
function SafarSectionRenderer({ sections }: { sections: SafarSection[] }) {
  return (<>{sections.map(s => <SectionBlock key={s.id} section={s} />)}</>);
}

function SectionBlock({ section }: { section: SafarSection }) {
  switch (section.type) {
    case 'text':
    ...
    case 'dalil':
      return (<div><h4 className="font-extrabold text-sm text-white/85 mb-2">{section.title}</h4>{section.dalil && (
        <div className="rounded-xl bg-cyan-500/5 border-l-2 border-cyan-400 p-3">
          ...
          {section.dalil.source && <p className="text-xs text-cyan-400/60 mt-2 font-bold">{section.dalil.source}</p>}
        </div>)}</div>);
    case 'checklist':
      return (<div><h4 className="font-extrabold text-sm text-white/85 mb-2">{section.title}</h4><ul className="space-y-2">
        {section.items?.map((item, i) => (<li key={i} className="flex items-start gap-2.5 text-sm text-white/50"><CheckCircle2 size={14} className="text-cyan-400 mt-0.5 shrink-0" />{item}</li>))}
      </ul></div>);
    case 'numbered-list':
    ...
    case 'madhab-table':
      return (<div><h4 className="font-extrabold text-sm text-white/85 mb-2">{section.title}</h4>{section.madhabTable && (
        <div className="overflow-x-auto rounded-xl border border-white/[0.06]">
          <table className="w-full text-sm">
            <thead><tr className="bg-white/[0.03]">{section.madhabTable.headers.map((h, i) => (<th key={i} className="px-3 py-2.5 text-left font-extrabold text-white/50 text-xs">{h}</th>))}</tr></thead>
            <tbody>{section.madhabTable.rows.map((row, i) => (<tr key={i} className="border-t border-white/[0.04] hover:bg-white/[0.02]"><td className="px-3 py-2.5 font-bold text-white/70 text-xs">{row.madhab}</td><td className="px-3 py-2.5 text-white/45 text-xs leading-relaxed">{row.opinion}</td></tr>))}</tbody>
          </table></div>)}</div>);
    case 'info-box': {
      const IconComponent = infoIcon(section.infoType);
      const isWarn = section.infoType === 'warning', isTip = section.infoType === 'tip';
      return (<div>{section.title && <h4 className="font-extrabold text-sm text-white/85 mb-2">{section.title}</h4>}
        <div className={`rounded-xl border p-4 ${isWarn ? 'bg-red-500/5 border-red-400/20' : isTip ? 'bg-cyan-500/5 border-cyan-400/20' : 'bg-blue-500/5 border-blue-400/20'}`}>
          <div className="flex items-start gap-2.5">
            <IconComponent size={15} className={`${isWarn ? 'text-red-400' : isTip ? 'text-cyan-400' : 'text-blue-400'} mt-0.5 shrink-0`} />
            <div className="space-y-2">{section.content && <p className="text-sm text-white/60 leading-relaxed">{section.content}</p>}
              ...
              {section.transliteration && <p className="text-xs text-cyan-400 italic">{section.transliteration}</p>}
    ...
    case 'arabic-dua':
      return (<div><h4 className="font-extrabold text-sm text-white/85 mb-2">{section.title}</h4>
        <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-4 space-y-3">
          ...
          {section.transliteration && <p className="text-sm text-cyan-400 italic leading-relaxed">{section.transliteration}</p>}
    ...
    case 'qa':
      return (<div><h4 className="font-extrabold text-sm text-white/85 mb-2">{section.title}</h4><div className="space-y-3">
        {section.qas?.map((qa, i) => (<div key={i} className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-3">
          <div className="flex items-start gap-2"><span className="text-xs font-extrabold text-cyan-400 bg-cyan-500/10 px-2 py-0.5 rounded-full shrink-0">T</span><p className="text-sm font-bold text-white/80">{qa.question}</p></div>
          <div className="flex items-start gap-2 mt-2 pl-1"><span className="text-xs font-extrabold text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full shrink-0">J</span><p className="text-sm text-white/50 leading-relaxed">{qa.answer}</p></div>
        </div>))}
      </div></div>);
    default: return null;
  }
