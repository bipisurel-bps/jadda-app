...
import { Coins, Wheat, ShoppingCart, Heart, ChevronDown, CheckCircle, AlertCircle } from 'lucide-react';
import { hitungZakatMaal, hitungZakatFitrah, hitungZakatPerdagangan, hitungZakatPertanian, formatCurrency, parseCurrency } from '@/lib/zakat';
import type { ZakatResult } from '@/lib/zakat';
import { toast } from 'sonner';

type TabType = 'maal' | 'fitrah' | 'perdagangan' | 'pertanian';

const tabs: { id: TabType; label: string; icon: React.ElementType; color: string }[] = [
  { id: 'maal', label: 'Zakat Maal', icon: Coins, color: 'text-primary' },
  { id: 'fitrah', label: 'Zakat Fitrah', icon: Heart, color: 'text-pink-500' },
  { id: 'perdagangan', label: 'Zakat Perdagangan', icon: ShoppingCart, color: 'text-blue-500' },
  { id: 'pertanian', label: 'Zakat Pertanian', icon: Wheat, color: 'text-amber-600' },
];
  ...
  const handleHitung = () => {
    ...
    switch (activeTab) {
      case 'maal':
        ...
        res = hitungZakatMaal({ totalHarta: parseCurrency(maalHarta), hutang: parseCurrency(maalHutang) });
        break;
      case 'fitrah':
      ...
      case 'perdagangan':
        ...
        res = hitungZakatPerdagangan({
          ...
          piutangLancar: parseCurrency(dagangPiutang), hutangDagang: parseCurrency(dagangHutang), stokBarang: parseCurrency(dagangStok)
        });
        break;
      case 'pertanian':
      ...
      {result && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
          ...
          <div className="text-xs text-muted-foreground/70 bg-muted/30 rounded-lg p-3 mt-2">
            <strong>Catatan:</strong> Perhitungan ini bersifat estimasi. Harga emas dan nisab dapat berubah. Konsultasikan dengan ulama atau lembaga amil zakat terpercaya untuk perhitungan yang lebih akurat.
          </div>
        </motion.div>
      )}
    </div>
